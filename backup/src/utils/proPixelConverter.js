...
import { perlerColors, colorDistance, ciede2000 } from './perlerColors.js';
...
export function proImageToPixelGrid(imageData, width, height, colorCount, targetSize = 50) {
  ...
  // 1. 双线性插值缩放 - 保留边缘细节
  const scaledData = bilinearScale(imageData, width, height, targetSize);
  const scaledWidth = targetSize;
  const scaledHeight = Math.round(targetSize * height / width);
  
  // 2. 颜色量化 - 使用CIEDE2000色差算法
  const palette = generateOptimalPalette(scaledData, scaledWidth, scaledHeight, colorCount);
  
  // 3. Floyd-Steinberg抖动 - 模拟平滑过渡
  const ditheredGrid = applyAdvancedDithering(scaledData, scaledWidth, scaledHeight, palette);
  
  // 4. 细节增强 - 特别针对小眼睛、线条嘴巴等
  const enhancedGrid = enhanceSmallDetails(ditheredGrid, palette);
  
  // 5. 统计颜色使用
  const colors = countColors(enhancedGrid);
  
  return { grid: enhancedGrid, colors };
}
