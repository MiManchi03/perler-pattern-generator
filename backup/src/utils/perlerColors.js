...
export function findClosestColor(rgb, colors = perlerColors) {
  ...
  // 关键改进：对于暗色，尝试匹配到更合适的颜色
  // 避免右边细节因为颜色匹配问题变淡
  const luminance = (rgb[0] * 299 + rgb[1] * 587 + rgb[2] * 114) / 1000;
  if (luminance < 40) { // 很暗的颜色
    // 尝试找更暗的拼豆颜色
    let darkestColor = closestColor;
    let darkestLum = (darkestColor.rgb[0] * 299 + darkestColor.rgb[1] * 587 + darkestColor.rgb[2] * 114) / 1000;
    
    for (const color of colors) {
      const colorLum = (color.rgb[0] * 299 + color.rgb[1] * 587 + color.rgb[2] * 114) / 1000;
      if (colorLum < luminance + 20 && colorLum < darkestLum) {
        const distance = colorDistance(rgb, color.rgb);
        if (distance < minDistance * 1.5) { // 允许稍大的距离
          darkestColor = color;
          darkestLum = colorLum;
        }
      }
